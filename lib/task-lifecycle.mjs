...
/** Resolve the absolute path a task file would occupy in a given folder. */
function taskPath(rootDir, context, folder, id) {
  return path.join(rootDir, '.agentheim', 'contexts', context, folder, `${id}.md`);
}
...
function dependencySatisfied(rootDir, depId) {
  ...
  for (const bc of entries) {
    const candidate = path.join(contextsDir, bc, 'done', `${depId}.md`);
...
export function applyTaskMove(rootDir, id, from, to, options = {}) {
  ...
  const toPath = taskPath(rootDir, context, to, id);
