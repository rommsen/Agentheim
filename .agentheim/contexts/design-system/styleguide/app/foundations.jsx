...
// ---- Doc scaffolding ----
function GuideSection({ index, title, desc, children }) {
  return (
    <section style={{ marginBottom: 72 }}>
      ...
      {desc && <p style={{ margin: "0 0 24px", maxWidth: 640, fontFamily: "var(--font-ui)", fontSize: 14.5, lineHeight: 1.6, color: "var(--fg-2)" }}>{desc}</p>}
      {children}
    </section>
  );
}
...
function DocCard({ children, style, pad = 24 }) {
  return (
    <div style={{
...
// ---- Color swatch ----
function Swatch({ varName, label, role, ring = false }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
      ...
      <div>
        ...
        {role && <div style={{ fontFamily: "var(--font-ui)", fontSize: 11, color: "var(--fg-3)", marginTop: 5, lineHeight: 1.4 }}>{role}</div>}
      </div>
    </div>
  );
}
...
function SemanticRow({ swatchColor, tintColor, label, token, sample }) {
    ...
    <GuideSection index="01" title="Color"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 26 }}>
        <div>
          ...
          <DocCard pad={20}>
            <SemanticRow swatchColor="--st-backlog" tintColor="--st-backlog-tint" label="Backlog" sample={<StatusChip status="backlog" />} />
            <SemanticRow swatchColor="--st-todo" tintColor="--st-todo-tint" label="To do" sample={<StatusChip status="todo" />} />
            <SemanticRow swatchColor="--st-doing" tintColor="--st-doing-tint" label="Doing" sample={<StatusChip status="doing" />} />
            <div style={{ marginBottom: -11 }}>
              <SemanticRow swatchColor="--st-done" tintColor="--st-done-tint" label="Done" sample={<StatusChip status="done" />} />
            </div>
          </DocCard>
        </div>
        <div>
          ...
          <DocCard pad={20}>
            <SemanticRow swatchColor="--ct-ticket" tintColor="--ct-ticket-tint" label="Ticket" sample={<TypePill type="ticket" />} />
            <SemanticRow swatchColor="--ct-context" tintColor="--ct-context-tint" label="Bounded context" sample={<TypePill type="context" />} />
            <SemanticRow swatchColor="--ct-vision" tintColor="--ct-vision-tint" label="Vision" sample={<TypePill type="vision" />} />
            <SemanticRow swatchColor="--ct-map" tintColor="--ct-map-tint" label="Context map" sample={<TypePill type="map" />} />
            <SemanticRow swatchColor="--ct-research" tintColor="--ct-research-tint" label="Research" sample={<TypePill type="research" />} />
            <div style={{ marginBottom: -11 }}>
              <SemanticRow swatchColor="--ct-adr" tintColor="--ct-adr-tint" label="ADR" sample={<TypePill type="adr" />} />
            </div>
          </DocCard>
        </div>
      </div>
...
Object.assign(window, { ThemeCtx, Glyph, useHex, GuideSection, SubHead, DocCard, Swatch, SwatchGrid, SemanticRow, ColorSection });
